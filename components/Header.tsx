...
function CartButton() {
  ...
  return (
    <button
      ...
      className="relative flex-none rounded-lg bg-tn-red px-3 py-1.5 text-[11px] font-black uppercase tracking-wide text-tn-white transition-all duration-200 hover:scale-105 hover:bg-tn-amber hover:text-tn-black sm:px-4 sm:py-2 sm:text-xs"
    >
  ...
  return (
    <header className="sticky top-0 z-40 border-b-4 border-tn-amber bg-tn-black">
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
        {/*
          The top bar only ever puts two short items side by side (logo,
          cart button) so it always fits on one line regardless of device —
          French UI text renders wider on some Android browsers than on iOS
          at the same viewport width, which used to make the nav links wrap
          unpredictably and blow up the header's height on Android. Search
          and the Accueil/Catalogue links get their own dedicated full-width
          rows below instead of competing for space on this line.
        */}
        <div className="flex items-center justify-between gap-3">
          ...
          {/* Desktop/tablet: search + nav links + cart all inline */}
          <div className="hidden flex-1 items-center justify-end gap-6 sm:flex">
            <form onSubmit={handleSearch} role="search" className="w-full max-w-xs flex-1">
            ...
            <nav className="flex flex-none items-center gap-6 text-sm font-bold uppercase tracking-wide text-tn-white">
              <Link href="/" className="transition-colors duration-200 hover:text-tn-amber">
                Accueil
              </Link>
              <Link href="/catalogue" className="transition-colors duration-200 hover:text-tn-amber">
                Catalogue
              </Link>
              <CartButton />
            </nav>
          ...
          {/* Mobile: just the cart button next to the logo */}
          <div className="sm:hidden">
            <CartButton />
          </div>
        </div>

        {/* Mobile-only: Accueil/Catalogue on their own row — always fits,
            never competes with the cart button or the search bar. */}
        <nav className="mt-3 flex items-center gap-5 text-xs font-bold uppercase tracking-wide text-tn-white sm:hidden">
          <Link href="/" className="transition-colors duration-200 hover:text-tn-amber">
            Accueil
          </Link>
          <Link href="/catalogue" className="transition-colors duration-200 hover:text-tn-amber">
